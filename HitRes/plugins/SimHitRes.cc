...
// Class:      SimHitRes
// 
/**\class TrackingAnalysis/Cosmics SimHitRes.cc TrackingAnalysis/Cosmics/src/SimHitRes.cc

 Description: <one line class summary>
Use overlaps in TIF cosmics data to evaluate hit resolution

 Implementation:
See sample cfg files in TrackingAnalysis/Cosmics/test/SimHitRes*cfg
*/
//
// Original Authors:  Wolfgang Adam, Keith Ulmer
//         Created:  Thu Oct 11 14:53:32 CEST 2007
// $Id: SimHitRes.cc,v 1.18 2010/05/19 15:31:58 speer Exp $
//
//
...
//
SimHitRes::SimHitRes(const edm::ParameterSet& iConfig) :
  config_(iConfig),
  trackerGeometry_(0),
  magField_(0),
  //trajectoryCollection(0),
...
void SimHitRes::load(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  using namespace edm;
  ...
  //iEvent.getByLabel(trackTag_.label(),tracksHandle);
...
void
SimHitRes::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
void SimHitRes::getHitParameters(const TransientTrackingRecHit::ConstRecHitPointer& hit, const LocalVector& trackDirection, unsigned int * detID, unsigned int * clusterWidth, float * expWidth, float * res, bool * atEdge, float * pitch){
  ...
  cout << "MatchedRecHit= " << matchedhit<< endl;
  if(matchedhit){
    cout << endl << "WHAT THE HELL!" << endl;
  ...
  cout << "TrueRes is = " << (*res) << endl;

}



float SimHitRes::getSimHitRes(const GeomDetUnit * det, const LocalVector& trackdirection, const TrackingRecHit& recHit, float * trackWidth, float *pitch){
  ...
  if(genTruth){
    ...
    cout << "RE to Sim hit matched= " << matched.empty() << endl;
    if (!matched.empty()) {
...
double SimHitRes::getSimpleRes(const TrajectoryMeasurement* traj1){
...
bool SimHitRes::getPairParameters(const TrajectoryMeasurement* traj1, const TrajectoryMeasurement* traj2, float * pairPath, float * hitDX, float * trackDX, float * trackDXE,
    float * trackParamX, float *trackParamY , float * trackParamDXDZ, float *trackParamDYDZ , float * trackParamXE, float *trackParamYE, float * trackParamDXDZE, float *trackParamDYDZE){
  (*pairPath) = 0;
  (*hitDX) = 0;
  (*trackDX) = 0;
...
//  if (abs(recHitX_1)>5) cout << "BAD: Bad hit position: Id = " << firstRecHit->geographicalId().rawId() << endl;
//  if (abs(recHitX_1)>5) cout << "BAD: Bad hit position: Id = " << secondRecHit->geographicalId().rawId() << endl;
...
int
SimHitRes::layerFromId (const DetId& id) const
{
