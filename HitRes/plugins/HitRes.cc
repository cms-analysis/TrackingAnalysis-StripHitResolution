...
HitRes::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  TrackerHitAssociator::Config helper(config_,consumesCollector());
  
  if(doSimHit_) associator = new TrackerHitAssociator(iEvent, helper); else associator = 0; 
		     ...
		     TrackerHitAssociator& associator)
{
  ...
  for ( OverlapContainer::const_iterator iol=overlapHits.begin();
	...
	const PixelTopology * topol =
	    dynamic_cast<const PixelTopology*>(&(theGeomDet->specificTopology()));
